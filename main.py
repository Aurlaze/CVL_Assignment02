...
# simple math helper to calculate how much the two boxes overlap (IoU)
...
# this function pops up the UI so we can manually draw the ground truth boxes
def manual_annotation(image_path):
    print(f"    [?] Missing ground truth for '{image_path}'. Opening the drawing tool...")
    ...
    # using a list here so the nested function below can modify it
    box_coords = [0, 0, 10, 10]

    # fires whenever we finish drawing the rectangle on the screen
    def onselect(eclick, erelease):
        x1, y1 = eclick.xdata, eclick.ydata
        x2, y2 = erelease.xdata, erelease.ydata
        
        # update our box coordinates with whatever we just drew
    ...
    # set up the matplotlib rectangle selector
    selector = RectangleSelector(
        ...
        spancoords='pixels',
        interactive=True # lets us grab the corners and resize the box
    )
